...
function VbsDecode() {
    ...
    this.unpackArray = function(value, arr) {
        ...
        // console.log(111, obj.toString(),arr)
        let pos = 0, j=0;
        let childObj = {};
    ...
    this.unpackObject = function(value) {
        ...
        let n = value.length;
        for (let i=0;i<n;i++) {
            if (value[i] == kindConst.vbsKind.VBS_DICT) {
                [valueObj, i, j] = unpackContentObj(value, n, i, j);
            } else {
                if ((value[i]&0x20) == kindConst.vbsKind.VBS_STRING) {  
                    let len = (value[i] & 0x1F);
                    keyStr = decode(value.slice(i, i+len+1));  
                    // console.log(111, value[i], value.slice(i, i+len+1), keyStr)
                    i += len+1;
                    if (value[i] == kindConst.vbsKind.VBS_DICT) {
                        [newObj, i, j] = unpackContentObj(value, n, i, j);
                        for(var key  in newObj){  
                            eval("obj['"+keyStr+"']="+JSON.stringify(newObj[key])); 
                        }
                    } else {
                        len = (value[i] & 0x1F);
                         // console.log(5555, i, len, value[i], value.slice(i+2, i+len+1))
                        valueObj = decode(value.slice(i, i+len+1));
                        console.log(1111, keyStr, valueObj, value.slice(i, i+len+1))
                        i += len;
                         
                        // eval("obj[" + keyStr + "]=" + valueObj); 
                        eval("obj['"+keyStr+"']="+valueObj);  
                    }
    ...
    function unpackContentObj(value, n, i, j) { // child object
        let newObj;
        for (j = i;j<n;j++) { // get the object content
